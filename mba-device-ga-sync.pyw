"""Script to synchronize chromebook information from MBA Device Manager+ in PowerSchool to Google Admin.

https://github.com/Philip-Greyson/D118-MBA-Device-GA-Sync
                    ...
                    if device[3] == "Chromebook" and device[4] != 1:  # filter to just non-archived Chromebooks
                        ...
                        if ga_status:  # if we found a match for the serial in the dictionary we can continue, otherwise need to throw an error for device not found
                            ...
                            if ps_status in Disabled_Statuses:
                                print(f'DBUG: Device should be disabled: Device Name: {name} | Device Serial: {serial} | PS Status: {ps_status}')
                                # print(f'DBUG: Device should be disabled: Device Name: {name} | Device Serial: {serial} | PS Status: {ps_status}', file=log)
                                ...
                                if ga_status != 'DISABLED' and ga_status != 'DEPROVISIONED':  # only disable if they are not already disabled, and cant do any actions on already deprovisioned devices until they get re-enrolled
                                    print(f'INFO: Device {serial} at {location} is currently {ga_status}, marked as "{ps_status}" in PS, and will be disabled. GA ID: {ga_device_id}')
                                    print(f'INFO: Device {serial} at {location} is currently {ga_status}, marked as "{ps_status}" in PS, and will be disabled. GA ID: {ga_device_id}', file=log)
                                    ga_device_id = deviceIDDict.get(serial)
                                    try:
                                    ...
                                    print(f'DBUG: Device {serial} is already disabled')
                                    # print(f'DBUG: Device {serial} is already disabled', file=log)


                            elif ps_status in Deprovision_Statuses:
                                print(f'DBUG: Device should be deprovisioned: Device Name: {name} | Device Serial: {serial} | PS Status: {ps_status}')
                                # print(f'DBUG: Device should be deprovisioned: Device Name: {name} | Device Serial: {serial} | PS Status: {ps_status}', file=log)
                                ga_device_id = deviceIDDict.get(serial)
                                if ga_status != 'DEPROVISIONED':  # only deprovision if they are not already deprovisioned
                                    print(f'INFO: Device {serial} at {location} is currently {ga_status}, marked as "{ps_status}" in PS, and will be deprovisioned. GA ID: {ga_device_id}')
                                    print(f'INFO: Device {serial} at {location} is currently {ga_status}, marked as "{ps_status}" in PS, and will be deprovisioned. GA ID:{ga_device_id}', file=log)
                                    try:
                                    ...
                                    print(f'DBUG: Device {serial} is already deprovisioned')
                                    # print(f'INFO: Device {serial} is already deprovisioned', file=log)

                            elif ps_status in Enabled_Statuses:
                                print(f'DBUG: Device should be activated: Device Name: {name} | Device Serial: {serial} | PS Status: {ps_status}')
                                # print(f'DBUG: Device should be activated: Device Name: {name} | Device Serial: {serial} | PS Status: {ps_status}', file=log)
                                ga_device_id = deviceIDDict.get(serial)
                                if ga_status != 'ACTIVE' and ga_status != 'DEPROVISIONED':  # only activate if they are not already active (disabled), also can't do anything to deprovisioned devices until re-enrolled
                                    print(f'INFO: Device {serial} at {location} is currently {ga_status}, marked as "{ps_status}" in PS, and will be re-enabled. GA ID: {ga_device_id}')
                                    print(f'INFO: Device {serial} at {location} is currently {ga_status}, marked as "{ps_status}" in PS, and will be re-enabled. GA ID:{ga_device_id}', file=log)
                                    try:
                                    ...
                                    print(f'INFO: Device {serial} is already activated')
                                    # print(f'INFO: Device {serial} is already activated', file=log)
